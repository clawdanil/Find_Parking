  ...
  spotsWithCoords.forEach((s, i) => {
    ...
    const popup = `
      <div style="padding:14px 16px;font-family:'Inter',sans-serif;min-width:200px;">
        <div style="font-size:0.68rem;font-weight:600;color:rgba(28,17,7,.35);letter-spacing:1.2px;text-transform:uppercase;margin-bottom:5px;">Spot ${num}</div>
        <div style="font-size:0.93rem;font-weight:600;color:#1C1107;line-height:1.35;margin-bottom:4px;">${escHtml(s.address)}</div>
        <div style="font-size:0.78rem;color:rgba(28,17,7,.50);margin-bottom:8px;">${escHtml(s.side)}</div>
        ${s.landmark ? `<div style="font-size:0.75rem;color:rgba(28,17,7,.35);font-style:italic;margin-bottom:8px;">📌 ${escHtml(s.landmark)}</div>` : ''}
        ...
        ${s.distance_from_search ? `<div style="font-size:0.73rem;color:rgba(28,17,7,.35);margin-top:8px;">📍 ${escHtml(s.distance_from_search)}</div>` : ''}
      </div>`;
