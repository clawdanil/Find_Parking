...
function toggleChat() {
  ...
  document.getElementById('chat-btn')?.setAttribute('aria-expanded', String(chatOpen));

  if (chatOpen && chatHistory.length === 0) {
    const loc = selectedCity || streetInput.value.split(',')[0]?.trim() || '';
    const greeting = loc
      ? `Hey! I'm Orbi, your local guide for ${loc}. Ask me anything — best spots to eat, bars with happy hour, parking tips, transit routes, or what's on tonight. What do you need?`
      : `Hey! I'm Orbi, your local guide. Search for a location first, then I can pull up real-time results for food, bars, parking, transit, events — whatever you need. What's on your mind?`;
    pushChatMsg('assistant', greeting);
  }
  if (chatOpen) setTimeout(() => chatInput?.focus(), 260);
}
...
function renderChatThread() {
  ...
  chatThread.innerHTML = chatHistory.map((m, i) => `
    <div class="chat-msg chat-msg-${m.role}" id="cmsg-${i}">
      ${m.role === 'assistant' ? '<div class="chat-orbi-icon">✦</div>' : ''}
...
function showTyping() {
  ...
  el.innerHTML = '<div class="chat-orbi-icon">✦</div><div class="chat-bubble chat-typing-bubble"><span class="tdot"></span><span class="tdot"></span><span class="tdot"></span></div>';
...
async function sendChat() {
  ...
  const wDesc = document.getElementById('w-desc')?.textContent;
  const wTemp = document.getElementById('w-temp')?.textContent;
  const weather = wDesc && wDesc !== 'Loading…' ? `${wTemp || ''} ${wDesc}`.trim() : null;
  ...
  try {
    const res = await fetch('/api/chat', {
      ...
      body: JSON.stringify({
        ...
        lat:      selectedLat,
        lng:      selectedLon,
        location: selectedCity || streetInput.value.split(',')[0]?.trim() || '',
        weather,
      }),
    });
    ...
    // Add empty assistant slot and render it
    chatHistory.push({ role: 'assistant', content: '' });
    renderChatThread();

    // Read SSE stream and update the last bubble in real time
      ...
      for (const line of lines) {
        ...
        try {
          ...
          if (ev.type === 'content_block_delta' && ev.delta?.type === 'text_delta') {
            chatHistory[chatHistory.length - 1].content += ev.delta.text;
            const bubble = document.querySelector(`#cmsg-${chatHistory.length - 1} .chat-bubble`);
            if (bubble) bubble.textContent = chatHistory[chatHistory.length - 1].content;
            chatThread.scrollTop = chatThread.scrollHeight;
          }
    ...
    document.getElementById('chat-typing')?.remove();
    pushChatMsg('assistant', "Sorry, I hit a snag — try again in a moment!");
  }
...
document.addEventListener('click', e => {
  ...
  if (!chatPanel?.contains(e.target) && !btn?.contains(e.target)) toggleChat();
});
